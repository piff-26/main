            ...
            @else
                <div class="space-y-4">
                    @foreach ($transactions as $transaction)
                        ...
                        <div class="bg-white/10 border rounded-2xl overflow-hidden transition
                            ...
                            <div class="flex items-center justify-between px-6 py-4 border-b border-white/10">
                                ...
                                @if ($isPaid)
                                    <span class="bg-green-500/20 border border-green-500/50 text-green-400 text-xs font-semibold px-3 py-1 rounded-full">
                                        <i class="fas fa-check-circle mr-1"></i>Lunas
                                    </span>
                                @elseif ($isPending)
                                    <span class="bg-yellow-500/20 border border-yellow-500/50 text-yellow-400 text-xs font-semibold px-3 py-1 rounded-full">
                                        <i class="fas fa-clock mr-1"></i>Menunggu Verifikasi
                                    </span>
                                @elseif ($isFailed)
                                    <span class="bg-red-500/20 border border-red-500/50 text-red-400 text-xs font-semibold px-3 py-1 rounded-full">
                            ...
                            @if ($isPaid && $transaction->tickets->isNotEmpty())
                                <div class="px-6 py-4">
                                    <p class="text-gray-400 text-xs font-semibold uppercase tracking-wider mb-3">Detail Tiket</p>
                                    <div class="space-y-2">
                                        @foreach ($transaction->tickets as $ticket)
                                            <div class="flex items-center justify-between bg-white/5 rounded-xl px-4 py-2.5">
                                                <div>
                                                    <p class="text-white text-sm font-semibold">{{ $ticket->ticketCategory->name }}</p>
                                                    <p class="text-gray-500 text-xs font-mono">{{ $ticket->ticket_code }}</p>
                                                </div>
                                                <span class="text-gray-400 text-xs">{{ $ticket->ticketCategory->event->name ?? '-' }}</span>
                                            </div>
                                        @endforeach
                                    </div>
                                </div>
                            ...
                            @elseif ($isPending)
                                <div class="px-6 py-4">
                                    <div class="bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-4 flex items-start gap-3">
                                        ...
                                        <div>
                                            <p class="text-yellow-300 text-sm font-semibold">Pembayaran sedang diverifikasi</p>
                                            <p class="text-yellow-300/70 text-xs mt-1">Tim kami sedang memeriksa bukti pembayaran Anda. Proses verifikasi biasanya 1x24 jam.</p>
                                        </div>
                                    </div>

                                    {{-- Item tiket yang dipesan --}}
                                    @if ($transaction->transactionItems->isNotEmpty())
                                        <p class="text-gray-400 text-xs font-semibold uppercase tracking-wider mt-4 mb-2">Tiket Dipesan</p>
                                        <div class="space-y-2">
                                            @foreach ($transaction->transactionItems as $item)
                                                <div class="flex items-center justify-between bg-white/5 rounded-xl px-4 py-2.5">
                                                    <div>
                                                        <p class="text-white text-sm font-semibold">{{ $item->ticketCategory->name }}</p>
                                                        <p class="text-gray-500 text-xs">{{ $item->quantity }}x tiket</p>
                                                    </div>
                                                    <span class="text-gray-400 text-sm">Rp {{ number_format($item->price * $item->quantity, 0, ',', '.') }}</span>
                                                </div>
                                            @endforeach
                                        </div>
                                    @endif
                                    ...
                                    <div class="bg-red-500/10 border border-red-500/20 rounded-xl p-4 flex items-start gap-3">
                                        ...
                                        <div>
                                            ...
                                            @if ($transaction->rejection_reason)
                                                <p class="text-red-300/70 text-xs mt-1">{{ $transaction->rejection_reason }}</p>
                                            @endif
                                            <p class="text-gray-500 text-xs mt-2">Kuota tiket Anda telah dikembalikan. Silakan lakukan pembelian ulang.</p>
                                        </div>
                                    </div>
                                </div>
                            @endif
                            ...
                            <div class="flex items-center justify-between px-6 py-4 border-t border-white/10">
                                <div>
                                    ...
                                    <p class="text-yellow-400 font-bold text-lg">Rp {{ number_format($transaction->total_amount, 0, ',', '.') }}</p>
                                </div>
                                ...
                                @elseif ($isPending)
                                    <span class="flex items-center gap-2 bg-yellow-500/20 text-yellow-400 text-sm px-5 py-2.5 rounded-full font-semibold">
